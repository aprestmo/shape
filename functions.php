...
// Part 1
function shape_fitvids() {
	if (!is_admin()) {

		wp_register_script( 'fitvids', get_template_directory_uri() . '/js/fitvids.js', array('jquery'), '1.0', true);    	
...
add_action('init', 'shape_fitvids');

/**
 * Automaticly append .video class to oembed content (not a perfect solution, but close)
 */

// Part 2
function shape_embed_filter( $html, $data, $url ) {
	
	$return = '<div class="video">' . $html . '</div>';
	return $return;
}
add_filter( 'oembed_dataparse', 'shape_embed_filter', 90, 3 );
